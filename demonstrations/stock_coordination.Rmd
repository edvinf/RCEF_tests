...
aggregateActivityDomains <- function(RCEF, desiredDomains){
  ...
  # 
  if (!("varMeanWeightLive" %in% names(estimates))){
    estimates$varMeanWeightLive <- as.numeric(NA)
  }
  if (!("meanWeightLive" %in% names(estimates))){
    estimates$meanWeightLive <- as.numeric(NA)
  }
...
desiredDomains <- data.table::data.table(activityDomain=c("Q1","Q2","Q3","Q4"), quarter=c(1,2,3,4))
...
sum(ICconverted$ESTIMATES$totalNumber) - sum(ICaggregated$ESTIMATES$totalNumber)
```
